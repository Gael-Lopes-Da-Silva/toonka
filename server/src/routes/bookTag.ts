...
router.get("/:id?", authentification, async (request, response) => {
	if (request.params.id) {
		const bookTag = (
			await db
				.select()
				.from(schema.bookTag)
				.where(eq(schema.bookTag.id, request.params.id))
		)[0];
	...
	filterEqual(request.query.bookId, schema.bookTag.bookId);
	filterEqual(request.query.name, schema.bookTag.name);
	...
	filterNullable(request.query.createdAt, schema.bookTag.createdAt);
	filterNullable(request.query.deletedAt, schema.bookTag.deletedAt);
	filterNullable(request.query.modifiedAt, schema.bookTag.modifiedAt);

	let query: any = db.select().from(schema.bookTag);
	...
	if (
		...
		query = query.orderBy(
			request.query.orderDirection === "desc"
				? desc(
						schema.bookTag[request.query.orderBy as (typeof orderMap)[number]],
					)
				: asc(
						schema.bookTag[request.query.orderBy as (typeof orderMap)[number]],
					),
...
router.put("/:id?", authentification, async (request, response) => {
	...
	let query: any = db.update(schema.bookTag).set({
		modifiedAt: new Date(),
		...(request.body.bookId !== undefined && {
			bookId: request.body.bookId,
		}),
